...
import { Container, Heading, Eyebrow, Lead, Placeholder, ButtonLink } from '@/components/ui';
...
export function Hero({
  ...
  const rhythm =
    variant === 'full'
      ? 'py-section-y xl:py-section-y-hero'
      : variant === 'page'
  ...
  return (
    <section id={id} className={`relative overflow-hidden bg-band text-on-band ${rhythm}`}>
      ...
      <Container className="relative">
        <div className={`grid gap-11 ${aside ? asideCols : ''} items-center`}>
          <div className="flex flex-col gap-7">
            {eyebrow ? <Eyebrow className="text-cta">{eyebrow}</Eyebrow> : null}
            <Heading level={1} as={1} display={display}>
              {title}
            </Heading>
            {subtitle ? <Lead className="text-on-band-muted">{subtitle}</Lead> : null}
